...
# "My Ryanair" — dedicated flight-assignment bot, staff only.
# Handles: /flight, /assign, /reassign, /report, /flightcancel, /flightupdate,
# and all the accept/decline/join button interactions that come with them.
...
async def assign_ticket_to_staff(guild, channel, user, tried_ids=None):
    ...
    try:
        e = discord.Embed(
            description=(f"Dear **{chosen.display_name}**,\n\nA new support ticket has been assigned to you.\n\n"
                         f"**User:** {user.display_name}\n**Ticket:** {channel.mention}\n\n"
                         f"Use `/connected` to claim it.\n\nIf not claimed, it transfers at <t:{transfer_time}:T>."),
...
async def ticket_reassign_monitor(channel, user, staff_id, tried_ids):
    ...
    try:
        e = discord.Embed(description=f"The ticket assigned to you ({channel.mention}) was not claimed within 30 minutes and has been reassigned.", color=RYANAIR_COLOR)
        e.set_footer(text="Ryanair Digital Assistant — Ticket Assignment")
        ...
        if owner:
            try:
                ...
                e = discord.Embed(
                    ...
                    description=(f"A staff member has been locked from moderation commands.\n\n"
                                 f"**Staff Member:** {user.display_name}\n**Username:** {str(user)}\n**User ID:** {uid}\n"
                                 f"**Reason:** {reason}\n**Misuse Count:** {mod_strike_count[uid]}\n\n"
                                 f"Use the buttons below to unlock or keep them locked."),
...
class FlightResponseView(discord.ui.View):
    ...
    async def not_join(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("You have been marked as not joining. You will receive a follow-up message.", ephemeral=True)
        ...
        try:
            e = discord.Embed(
                description=(f"Dear **{interaction.user.display_name}**,\n\nYou have declined the following flight:\n\n"
                             ...
                             f"If you genuinely could not attend, please do not worry — we just need an explanation for our records.\n\n"
                             f"Thank you for your understanding. Have a wonderful day!"),
                color=RYANAIR_COLOR, timestamp=now()
            )
    ...
    async def accept(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if guild and guild.owner:
            try:
                owner_user = await my_ryanair_bot.fetch_user(guild.owner.id)
                e = discord.Embed(
                    description=f"**{interaction.user.display_name}** has accepted their assignment.\n\n**Role:** {assignment.get('role','N/A')}\n**Flight:** {assignment.get('flight_num','N/A')}\n**Destination:** {assignment.get('destination','N/A')}",
                    color=0x57F287, timestamp=now()
                )
                e.set_footer(text="Ryanair Digital Assistant — Assignment")
                await owner_user.send(embed=e)
            except: pass
        fid = assignment.get("flight_id","")
        role_id = assignment.get("role_id","")
        if fid and role_id and fid in role_slot_counts:
            slots = role_slot_counts[fid].get(role_id, {})
            slots["accepted"] = slots.get("accepted", 0) + 1
            role_slot_counts[fid][role_id] = slots; save_data()
            limit = slots.get("limit", 0); accepted = slots.get("accepted", 0)
            if limit > 0 and accepted >= limit and guild and guild.owner:
                try:
                    role_obj = guild.get_role(int(role_id))
                    role_name = role_obj.name if role_obj else role_id
                    owner_user = await my_ryanair_bot.fetch_user(guild.owner.id)
                    e = discord.Embed(
                        description=f"All **{limit}** slots for role **{role_name}** on flight **{assignment.get('flight_num','N/A')}** have been filled!",
                        color=0x57F287, timestamp=now()
                    )
                    e.set_footer(text="Ryanair Digital Assistant — Role Slots Full")
                    await owner_user.send(embed=e)
                except: pass
            ...
            try:
                ...
                e = discord.Embed(
                    ...
                    description=(f"**{interaction.user.display_name}** has declined their assignment.\n\n"
                                 ...
                                 f"**Destination:** {assignment.get('destination','N/A')}\n**Report Time:** {assignment.get('report_time','N/A')}\n\n"
...
class ReportJoinView(discord.ui.View):
    ...
    async def join_now(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        await interaction.response.send_message("Great! Your attendance has been confirmed. See you on the flight!", ephemeral=True)
        guild = bot.get_guild(GUILD_ID)
        if guild and guild.owner:
            try:
                owner_user = await my_ryanair_bot.fetch_user(guild.owner.id)
...
class TicketChannelView(discord.ui.View):
    ...
    @discord.ui.button(label="Open a Ticket", style=discord.ButtonStyle.primary, custom_id="ticketchannel_open")
...
async def assignment_expiry_monitor(assignment_id, expires_dt):
    ...
    if guild and guild.owner:
        try:
            ...
            await asyncio.sleep(1800)
            assignment = assignments.get(assignment_id)
            if assignment and assignment.get("status") not in ["accepted","confirmed","cancelled"]:
                e2 = discord.Embed(
                    title="URGENT — Assignment Still Not Resolved",
                    description=(f"This assignment still has no confirmed response after 30 minutes.\n\n"
                                 f"**Flight:** {assignment.get('flight_num','N/A')} | **Role:** {assignment.get('role','N/A')}\n"
                                 f"**Report Time:** {assignment.get('report_time','N/A')}\n\n"
                                 f"Please run `/reassign {assignment_id} [member]` immediately."),
                    color=0xFF0000, timestamp=now()
                )
                e2.set_footer(text="Ryanair Digital Assistant — URGENT REMINDER")
                await owner_user.send(embed=e2)
        except: pass
...
@app_commands.describe(airline="ryanair, buzz, malta, lauda", title="Title", message="Body (use \\n for new lines, supports **bold** *italic* - lists)", image_url="Image URL shown above embed")
async def announce(interaction: discord.Interaction, airline: str, title: str, message: str, image_url: str = None):
    await interaction.response.defer(ephemeral=True)
    if not is_senior(interaction.user): await interaction.followup.send("Senior Staff+ only.", ephemeral=True); return
    style = AIRLINE_STYLES.get(airline.lower())
    if not style: await interaction.followup.send("Unknown airline. Use: ryanair, buzz, malta, lauda", ephemeral=True); return
    guild = bot.get_guild(GUILD_ID)
    ann_channel = guild.get_channel(ANNOUNCEMENT_CHANNEL_ID)
    if not ann_channel: await interaction.followup.send("Announcement channel not found.", ephemeral=True); return
    final_msg = message.replace("\\n", "\n")
    corrected_title = await autocorrect_text(title)
    if image_url: await ann_channel.send(image_url)
    e = discord.Embed(title=corrected_title, description=final_msg, color=ANNOUNCE_COLOR, timestamp=now())
    e.set_footer(text=f"{style['label']} | Announcement by {interaction.user.display_name}")
    await ann_channel.send(embed=e)
...
@app_commands.describe(channel="Channel to send to", airline="ryanair, buzz, malta, lauda", title="Title", message="Body (use \\n for new lines, supports **bold** *italic* - lists)", image_url="Image URL shown above embed")
async def announcechannel(interaction: discord.Interaction, channel: discord.TextChannel, airline: str, title: str, message: str, image_url: str = None):
    await interaction.response.defer(ephemeral=True)
    if not is_senior(interaction.user): await interaction.followup.send("Senior Staff+ only.", ephemeral=True); return
    style = AIRLINE_STYLES.get(airline.lower())
    if not style: await interaction.followup.send("Unknown airline. Use: ryanair, buzz, malta, lauda", ephemeral=True); return
...
@app_commands.describe(airline="ryanair, buzz, malta, lauda", title="Title", message="Body (use \\n for new lines)", image_url="Optional image URL", staff_only="Only DM staff members?")
async def notifydm_cmd(interaction: discord.Interaction, airline: str, title: str, message: str, image_url: str = None, staff_only: bool = False):
    await interaction.response.defer(ephemeral=True)
    if not is_lock(interaction.user): await interaction.followup.send("Owner only.", ephemeral=True); return
    style = AIRLINE_STYLES.get(airline.lower())
    if not style: await interaction.followup.send("Unknown airline. Use: ryanair, buzz, malta, lauda", ephemeral=True); return
    ...
    corrected_title = await autocorrect_text(title)
    sent = 0
    targets = [m for m in guild.members if not m.bot and (is_level1(m) if staff_only else True)]
    await interaction.followup.send(f"Sending to {len(targets)} members...", ephemeral=True)
    for member in targets:
        try:
            e = discord.Embed(title=corrected_title, description=final_msg, color=style["color"], timestamp=now())
            if image_url: e.set_image(url=image_url)
            e.set_footer(text=f"{style['label']} | Ryanair Digital Assistant")
            if image_url: await member.send(image_url)
            await member.send(embed=e); sent += 1; await asyncio.sleep(0.5)
        except: pass
    try: await interaction.user.send(embed=plain_embed(f"Notification sent to {sent} {'staff' if staff_only else 'members'}."))
    except: pass

@tree.command(name="embed", description="Send a custom embed to any channel (Senior Staff+)", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(channel="Channel", title="Title", message="Body (use \\n for new lines, supports **bold** etc)", colour="Hex colour e.g. 073590", image_url="Optional image URL")
async def embed_cmd(interaction: discord.Interaction, channel: discord.TextChannel, title: str, message: str, colour: str = "073590", image_url: str = None):
    await interaction.response.defer(ephemeral=True)
    if not is_senior(interaction.user): await interaction.followup.send("Senior Staff+ only.", ephemeral=True); return
    ...
    final_msg = message.replace("\\n", "\n")
    corrected_title = await autocorrect_text(title)
    e = discord.Embed(title=corrected_title, description=final_msg, color=color_int, timestamp=now())
    if image_url: await channel.send(image_url)
    e.set_footer(text=f"Ryanair Digital Assistant | Posted by {interaction.user.display_name}")
    await channel.send(embed=e)
...
@app_commands.describe(member="User", nickname="New nickname (blank to reset)", emoji_name="Optional server emoji name to prefix")
async def nick_cmd(interaction: discord.Interaction, member: discord.Member, nickname: str = None, emoji_name: str = None):
    ...
    try:
        ...
        if emoji_name:
            ...
            found = discord.utils.get(guild.emojis, id=int(match.group(2))) if match else discord.utils.get(guild.emojis, name=emoji_name.strip())
            if found: final_nick = f"{str(found)} {final_nick}".strip()
...
async def roleemoji_cmd(interaction: discord.Interaction, role: discord.Role, emoji_name: str):
    ...
    if not found:
        await interaction.followup.send(f"Could not find emoji `{stripped}` in this server. Paste it directly e.g. `<:myemoji:123456>` or type the name.", ephemeral=True); return
    try:
            ...
            try:
                e = discord.Embed(
                    ...
                    description=(f"**Airline:** {airline}\n**Flight:** {flight_num}\n**Route:** {destination}\n"
                                 ...
                                 f"Please use the buttons below to confirm your attendance.\n"
                                 f"If you decline, you will be required to open a Staff Hub ticket explaining why."),
                    color=RYANAIR_COLOR, timestamp=now()
                )
                if image_url: e.set_image(url=image_url)
...
@app_commands.describe(flight_id="The flight ID from /flight")
...
@tree.command(name="assign", description="Assign one or more staff members to a flight role (Owner only)", guild=discord.Object(id=GUILD_ID))
@app_commands.describe(
    flight_id="Flight ID (from /flight)",
    server_role="Server role for this assignment",
    member1="Staff member 1",
    member2="Staff member 2 (optional)",
    member3="Staff member 3 (optional)",
    member4="Staff member 4 (optional)",
    member5="Staff member 5 (optional)",
    report_time="Report time in UK time e.g. 1:00 PM",
    sign_out_time="Sign out time in UK time e.g. 5:00 PM",
    game_link="Link to the game airport",
    expires_at="Deadline to accept by UK time e.g. 12:00 PM",
    role_limit="How many of this role are needed (0 = unlimited)",
    give_role="Also give the server role to each member?"
)
async def assign_cmd(interaction: discord.Interaction, flight_id: str, server_role: discord.Role,
                     member1: discord.Member, report_time: str, sign_out_time: str,
                     game_link: str, expires_at: str,
                     member2: discord.Member = None, member3: discord.Member = None,
                     member4: discord.Member = None, member5: discord.Member = None,
                     role_limit: int = 0, give_role: bool = True):
    await interaction.response.defer(ephemeral=True)
    if not is_lock(interaction.user): await interaction.followup.send("Owner only.", ephemeral=True); return
    fid = flight_id.upper()
    flight = active_flights.get(fid)
    if not flight: await interaction.followup.send(f"Flight ID `{fid}` not found. Run `/flight` first.", ephemeral=True); return
    members = [m for m in [member1, member2, member3, member4, member5] if m is not None]
    report_dt  = parse_uk_time(report_time)
    expires_dt = parse_uk_time(expires_at)
    ...
    if str(server_role.id) not in role_slot_counts[fid]:
        ...
        role_slot_counts[fid][str(server_role.id)]["limit"] = role_limit
    save_data()
    sent = 0; assignment_ids = []
    for member in members:
        ...
        assignments[aid] = {
            ...
            "airline": flight.get("airline","N/A"), "report_time": report_time,
            "report_time_utc": report_dt.isoformat() if report_dt else None,
            "sign_out_time": sign_out_time, "game_link": game_link,
            "expires_at": expires_at, "expires_utc": expires_dt.isoformat() if expires_dt else None,
            "flight_id": fid, "status": "pending", "by": interaction.user.display_name, "time": now().isoformat(),
        }
        save_data(); assignment_ids.append(aid)
        if give_role:
            try: await member.add_roles(server_role, reason=f"Flight assignment {aid}")
            except: pass
        slots = role_slot_counts[fid].get(str(server_role.id), {})
        limit = slots.get("limit", 0); accepted = slots.get("accepted", 0)
        remaining_text = f"\n\n**Slots remaining for this role:** {limit - accepted}/{limit}" if limit > 0 else ""
        try:
            msg = (f"Dear **{member.display_name}**,\n\nYou have been assigned to the following flight:\n\n"
                   ...
                   f"**Airline:** {flight.get('airline','N/A')}\n**Route:** {flight.get('destination','N/A')}\n"
                   f"**Report Time (UK):** {report_time}\n**Sign Out Time (UK):** {sign_out_time}\n"
                   f"**Game Airport Link:** {game_link}\n\n"
                   f"You must accept this assignment by **{expires_at} UK time**.\n"
                   f"If there is no response, the owner will be notified to reassign.\n\n"
                   f"Once you accept, please join the game and run `!login` to log your attendance.\n\n"
            ...
            print(f"Failed to DM {member.display_name}: {ex}")
    ...
    try:
        confirm = discord.Embed(
            description=f"Assignment sent to **{len(members)} staff member(s)** as **{server_role.name}** for flight **{flight.get('flight_num','N/A')}**.\n\nAssignment IDs: {', '.join(f'`{aid}`' for aid in assignment_ids)}",
            color=RYANAIR_COLOR
        )
        confirm.set_footer(text="Ryanair Digital Assistant — Flight Assignment")
        await send_my_ryanair_dm(interaction.user.id, confirm)
    except: pass
    await interaction.followup.send(f"Assignment sent to {sent}/{len(members)} staff member(s).\nIDs: {', '.join(f'`{aid}`' for aid in assignment_ids)}", ephemeral=True)
...
async def reassign_cmd(interaction: discord.Interaction, assignment_id: str, new_member: discord.Member, new_role: discord.Role = None):
    ...
    try:
        msg = (f"Dear **{new_member.display_name}**,\n\nYou have been assigned to the following flight as a replacement:\n\n"
               ...
               f"**Game Airport Link:** {assignment.get('game_link','Check with owner')}\n\n"
               f"Please be in-game and at the airport by your report time.\n\nClick **Accept** below to confirm your attendance. Thank you!")
            ...
            if staff_id:
                try:
                    e = discord.Embed(title="Flight Cancelled",
                                      description=f"The following flight has been cancelled:\n\n**Flight:** {flight.get('flight_num','N/A')}\n**Route:** {flight.get('destination','N/A')}\n**Reason:** {reason}\n\nYou are no longer required for this flight. Thank you for your understanding.",
                                      color=0xFF0000, timestamp=now())
                    e.set_footer(text="Ryanair Digital Assistant — Flight Management")
                    user_obj = await my_ryanair_bot.fetch_user(staff_id)
                    await user_obj.send(embed=e); notified += 1
                except: pass
    for uid, r in flight_responses.get(fid, {}).items():
        if r == "joining":
            try:
                e = discord.Embed(description=f"The flight **{flight.get('flight_num',fid)}** has been cancelled.\n\n**Reason:** {reason}", color=0xFF0000, timestamp=now())
                e.set_footer(text="Ryanair Digital Assistant — Flight Management")
                user_obj = await my_ryanair_bot.fetch_user(int(uid))
                await user_obj.send(embed=e); notified += 1
            except: pass
...
async def config_cmd(interaction: discord.Interaction):
    ...
    e = discord.Embed(
        ...
        description=(
            "Use the buttons below to set the role for each level.\n\n"
            f"**Level 1 — Junior Staff** | Role: {rname(cfg.get('1'))}\n"
            f"Commands: Basic info, stafflist, botstatus, remind, viewtickets\n\n"
            f"**Level 2 — Mid Staff** | Role: {rname(cfg.get('2'))}\n"
            f"Commands: Level 1 + userinfo, notes, warnings, AI access\n\n"
            f"**Level 3 — Reserved** | Role: {rname(cfg.get('3'))}\n\n"
            f"**Ticket Access Role** | Role: {rname(cfg.get('ticket_role'))}\n"
            f"This role gets full ticket access same as Level 4\n\n"
            f"**Level 4 — Senior Staff** | Role: {rname(cfg.get('4'))}\n"
            f"Commands: Everything + Full tickets + Full moderation\n\n"
            f"**Level 5 — Owner** | Role: {rname(cfg.get('5'))}\n"
            f"Commands: Everything including flight, config and owner commands"
        ),
        color=RYANAIR_COLOR, timestamp=now()
    )
...
async def update_cmd(interaction: discord.Interaction):
    ...
    e.add_field(name="✈️ Flight System", value="`/flight` `/attended` `/assign` (multiple staff, role dropdown, slot limits) `/reassign` `/report` `/assigned` `/flightcancel` `/flightupdate`\nAll flight DMs are sent via **My Ryanair**", inline=False)
    e.add_field(name="📢 Announcements", value="`/announce` `/announcechannel` `/channelembed` `/notifydm` `/announcedm` `/embed`\nAll support `\\n` for new lines, **bold**, *italic*, - lists", inline=False)
    e.add_field(name="🤖 AI System", value="`/ai` `/aiask` `/aistatus` `/ai_toggle` `/ai_ticket_toggle` `/ai_preset_add` `/ai_preset_remove` `/aideal` `/ticketsummary`\nOwner can DM the bot directly to announce or DM staff via AI", inline=False)
...
async def commands_cmd(interaction: discord.Interaction, category: str = "all"):
    ...
    if category in ("tickets","all") and level >= 4:
        ...
        e.add_field(name="Senior Staff (Level 4+)", value="`/forceopen` `/ticketrename` `/tickettransfer` `/ticketpriority` `/ticketban` `/ticketunban` `/snippetadd` `/snippetdelete` `/pingstaff`", inline=False)
    ...
    if category in ("flight","all"):
        e = discord.Embed(title="✈️ Flight Commands", color=RYANAIR_COLOR)
        if level >= 5:
            e.add_field(name="Owner Only", value=("`/flight [flight_num] [destination] [airline] [departure_time] [report_time]` — Announce flight\n"
                                                  "`/attended [flight_id]` — View attendance\n"
                                                  "`/assign [flight_id] [role] [member1..5] [report_time] [sign_out_time] [game_link] [expires_at]` — Assign staff (up to 5, role dropdown, slot limits)\n"
                                                  "`/reassign [id] [member]` — Reassign a declined slot\n"
                                                  "`/report [flight_id]` — Send join now to assigned staff\n"
                                                  "`/assigned [flight_id]` — View all assignments\n"
                                                  "`/flightcancel [flight_id]` — Cancel a flight\n"
                                                  "`/flightupdate [flight_id] [field] [value]` — Update flight details\n\n"
                                                  "All of the above DM staff through **My Ryanair**, our dedicated flight bot."), inline=False)
        else:
            e.description = "Flight commands are Owner only."
        embeds.append(e)

    if category in ("announcements","all") and level >= 4:
        e = discord.Embed(title="📢 Announcement Commands", color=RYANAIR_COLOR)
        e.add_field(name="Senior Staff (Level 4+)", value=("`/announce [airline] [title] [msg]` — Main announcement channel\n"
                                                            "`/announcechannel [channel] [airline] [title] [msg]` — Any channel\n"
                                                            "`/channelembed [channel] [image_url]` — Post just an image\n"
                                                            "`/embed [channel] [title] [msg]` — Custom embed\n\n"
                                                            "Tip: Use `\\n` for new lines, supports **bold**, *italic*, - lists"), inline=False)
        if level >= 5: e.add_field(name="Owner Only", value="`/notifydm [airline] [title] [msg]` — DM everyone\n`/announcedm [msg]` — DM all staff", inline=False)
        embeds.append(e)

    if category in ("ai","all") and level >= 2:
        e = discord.Embed(title="🤖 AI Commands", color=RYANAIR_COLOR)
        e.add_field(name="Level 2+", value="`/ai` — Start private AI session in DMs\n`/aiask [question]` — Quick AI question", inline=False)
        ...
        if level >= 5: e.add_field(name="Owner Only", value="`/ai_toggle` — Enable/disable staff AI\n`/ai_ticket_toggle` — Globally enable/disable ticket AI\n`/ai_preset_add` `/ai_preset_remove` — Manage AI instructions\nDM the bot directly to use AI to announce or message staff", inline=False)
        embeds.append(e)
